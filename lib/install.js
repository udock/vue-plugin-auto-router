    ...
    if (!file || REGEX_VALIDATE_DIR.test(file)) {
      ...
      return readdir(parent)
        .then(function (entries) {
          ...
          return Promise.all(promises).then(function () { return Promise.resolve(result) })
        })
    }
  } else {
...
function createRoutes (files, options) {
  ...
  Object.keys(files).sort().forEach(function (file) {
    ...
    let route = {
      ...
      hasExOptions: PICK_REGEX.test(fs.readFileSync(files[file]))
    }
    let parent = routes
...
function optimizeAsyncImport (code, options) {
  if (options.settings) {
    const config = _.map(options.settings, function (item) {
      if (_.isString(item)) {
    ...
    return code.replace(REGEX_ASYNC_IMPORT, function (all, module) {
