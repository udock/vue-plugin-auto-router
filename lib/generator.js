...
var REGEX_MODULE_INDEX = /^(\/modules\/[\w-]+)*\/index\.vue$/i;
var REGEX_VALIDATE_DIR = /^\/(modules\/[\w-]+\/)*(modules(\/[\w-]+|)|pages)$/i;
var REGEX_VALIDATE_PAGE = /^(\/modules\/[\w-]+)*\/pages\/[\w-]+\.vue$/i;
...
function createRoutes(files, options) {
    ...
    Object.keys(files).sort().forEach(function (file) {
        var keys = file.replace(/\.vue$/, '').replace(/\/{2,}/g, '/').split('/').slice(1);
        var route = {
            name: '',
            path: '',
            component: './' + path_1.default.relative(options.contextPath, files[file]).replace(/\\/g, '/'),
            hasExOptions: config_1.default.PICK_REGEX.test(fs_1.default.readFileSync(files[file]) + '')
        };
        var parent = routes;
...
function optimizeAsyncImport(code, options) {
    if (options.settings) {
        var config_3 = lodash_1.default.map(options.settings, function (item) {
        ...
        return code.replace(REGEX_ASYNC_IMPORT, function (_, module) {
            var chunkName = getChunkName(path_1.default.resolve(options.contextPath, module), config_3);
...
var generateAutoRouters = function (options) {
    return new Promise(function (resolve, reject) {
        ...
        var templateFilePath = path_1.default.resolve(__dirname, '..', 'template', 'define.js');
    ...
    return __awaiter(this, void 0, void 0, function () {
        ...
        return __generator(this, function (_c) {
            ...
            return [2 /*return*/, walk(dir, {
                    ...
                    for (var _i = 0, _c = result.dirs; _i < _c.length; _i++) {
                        var item = _c[_i];
